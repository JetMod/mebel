<?php
	/* Template Name: Шаблон страницы Перетяжка мягкой мебели */
get_header();
?>
...
<section class="text-section">
	<div class="content base-text">
        ...
        <?php if (have_rows('advant_over_items')) : ?>
            <div class="icon-list">
            <?php while (have_rows('advant_over_items')) : the_row(); ?>
                <div>
                    <img src="<?php the_sub_field('icon_advant'); ?>" alt="">
                    <p><?php the_sub_field('text_advant'); ?></p>
                </div>
            <?php endwhile; ?>
            </div>
        <?php endif;?>
        ...
        <?php if (have_rows('gallery_over')) : ?>
        <div>
            <div class="gallery-block">
                <?php while(have_rows('gallery_over')) : the_row(); ?>
    ...
    <div class="content">
        ...
        <?php 
            for($i=0;$i<=17;$i++) {
                ...
                if (have_rows($table_price,ID_PAGE_OVERDRAWING)): ?>
                    <div class="grid-price-table">
                    <?php while (have_rows($table_price,ID_PAGE_OVERDRAWING)) : the_row();?>
...
<section class="min-section border-bottom-light">
    <div class="content">
        ...
        <div class="base-text">
            ...
            <div>
                <?php the_field('tekst_bloka_iz_chego_skladyvaetsya_czena');?>
            </div>
            <?php endif; ?>
...
<?php if (have_rows('overdrawing_types')) : ?>
<section class="text-section pb0 no-border-bottom">
    <div class="content base-text">
        <?php $i = 1;
        while (have_rows('overdrawing_types')) : the_row();
        if ($i % 2 == 0) :?>
...
<?php if (get_field('text_end_section_over')) :?>
<section class="text-section">
    <div class="content base-text">
        <?php the_field('text_end_section_over'); ?>
    </div>
</section>
<?php endif; ?>
...
<?php if (have_rows('foto_products_over')) :?>
<section class="base-section crem-fon border-bottom-dark">
    <div class="content">
        ...
        <div class="photos-block">
            <?php while (have_rows('foto_products_over')) : the_row(); ?>
